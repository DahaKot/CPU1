...
int CPU_Run () {
    ...
    for ( ; code[c_cmd] != EOP; c_cmd++) {
        if (code[c_cmd] == PUSH_NUMBER) {
        ...
        else if (code[c_cmd] == ADD) {
            CPU_DEBUG CLog_write(ADD, __LINE__);
            ...
            check = Stack_Push(&(cpu->stack), thing_1 + thing_2);
            if (check != ALL_RIGHT) {
                CPU_DEBUG CLog_write(STACK, __LINE__);
                return ERR;
            }
        }
        else if (code[c_cmd] == SUB) {
            CPU_DEBUG CLog_write(SUB, __LINE__);

            data_t lesser = Stack_Pop(&(cpu->stack));
            data_t thing = Stack_Pop(&(cpu->stack));
            if (lesser == ERR || thing == ERR || lesser == EMPTY || thing == EMPTY) {
                CPU_DEBUG CLog_write(STACK, __LINE__);
                return ERR;
            }
            check = Stack_Push(&(cpu->stack), thing - lesser);
            if (check != ALL_RIGHT) {
                CPU_DEBUG CLog_write(STACK, __LINE__);
                return ERR;
            }
        }
        else if (code[c_cmd] == MUL) {
            CPU_DEBUG CLog_write(MUL, __LINE__);
        ...
        else if (code[c_cmd] == DIV) {
            CPU_DEBUG CLog_write(DIV, __LINE__);

            data_t divisor = Stack_Pop(&(cpu->stack));
            data_t divident = Stack_Pop(&(cpu->stack));
            if (divisor == ERR || divident == ERR || divisor == EMPTY || divident == EMPTY) {
                CPU_DEBUG CLog_write(STACK, __LINE__);
                return ERR;
            }
            check = Stack_Push(&(cpu->stack), divident / divisor);
            if (check != ALL_RIGHT) {
                CPU_DEBUG CLog_write(STACK, __LINE__);
                return ERR;
            }
        }
        else if (code[c_cmd] == MOD) {
            CPU_DEBUG CLog_write(MOD, __LINE__);
            ...
            check = Stack_Push(&(cpu->stack), (int) divident % (int) divisor);
            if (check != ALL_RIGHT) {
                CPU_DEBUG CLog_write(STACK, __LINE__);
                return ERR;
            }
        }
        else if (code[c_cmd] == IDIV) {
            CPU_DEBUG CLog_write(IDIV, __LINE__);
